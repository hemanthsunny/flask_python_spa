...
from app.kudo.schema import RepoSchema
...
@login_required
def index():
    return json_response(KudoService(g.user).find_all())


@app.route("/kudos", methods=["POST"])
@login_required
def create():
    github_repo = RepoSchema().load(json.loads(request.data))

    if github_repo.errors:
        return json_response({'error': github_repo.errors}, 422)

    kudo = KudoService(g.user).create_kudo(github_repo)
    return json_response(kudo)

@app.route("/kudo/<int:repo_id>", methods=["GET"])
@login_required
def show(repo_id):
    kudo = KudoService(g.user).find_kudo(repo_id)
